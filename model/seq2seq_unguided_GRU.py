...
parser.add_argument('--minEpoch', type=int, default=10000, metavar='ME',
...
save_path_name = os.path.join(args.path,folder)
...
array_length = y_length
percentage = 0.75 # the percentage of data used for training
filename = args.fileName # "BN.txt"
name_flag = "Water_RMSE_GRU" # the name flag for the test case
...
#print(len(indices))
#sys.exit(0)
#print(X_train_data[int(indices[0])])
...
name = "GRU_"+str(x_length)+"_"+str(y_length)+"_real.txt"
name = os.path.join(save_path_name,name)
...
MIN_EPOCH_LIM = args.minEpoch # 1000
CONTINUE_FLAG = True
EPOCH_LIMIT = args.minEpoch # 50000
#epochs = 1000 # the number of epochs trained
#batch_size = 50 # not trained in batches
#random_train_epochs = 200 # maybe 200 iterations are enough
#keep_rate = 0.5
...
with tf.Session() as sess:
    ...
    while CONTINUE_FLAG:
        ...
        if ep % 100 == 0:
            temp_output = np.reshape(out_t,(array_length,-1))
    ...
    """temp_saver = rnn_model['saver']()
    temp_saver.restore(sess,os.path.join(save_path_name,save_object_name))"""
    ...
    #-- trying a new method
    #feed_dict2.update({rnn_model['target_seq'][t]:np.zeros([1,output_dim]) for t in range(y_length)})
    ...
    matrix = np.reshape(out_t,(array_length,-1))
...
name = "GRU_"+str(x_length)+"_"+str(y_length)+"_pred.txt" # "Seq2seq_unguided_"+name_flag+"_Y_found_x_"+str(x_length)+"_y_"+str(y_length)+"data.txt"
name = os.path.join(save_path_name,name)
writetofile(name,Y_found)

err = RMSE(Y_test_data,Y_found)

name = os.path.join(save_path_name,"test_rmse.csv")
...
#name = "Seq2seq_unguided_"+name_flag+"_plot_x_"+str(x_length)+"_y_"+str(y_length)+"data.jpg"
#fig.savefig(name)
#plt.close()
#close(fig)
#fig.close()
...
#plt.plot(train_loss)
#plt.ylabel("LOSS (MSE)")
#name =  "Seq2seq_unguided_"+name_flag+"_TRAIN_LOSS_plot_x_"+str(x_length)+"_y_"+str(y_length)+"data.jpg"
#fig.savefig(name)
#plt.close()
